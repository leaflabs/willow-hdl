...
from spi_flash import numonyx_flash_32MB
...
def usage():
  ...
  print ""
  print "usage: sng-control.py [options] <filename>"
  print ""
  ...
  print "\t\t\t\t: Note: Default is 0403:6010"
  ...
  print "\t\tsng-control.py bitfile.bin"
  print ""
  print "\treset the FPGA and program without loading a new bin file"
  print "\t\tsng-control.py -p"
  print ""
  print "\treset the FPGA and program without loading a new bin file on an alternate FTDI VID:PID"
  print "\t\tsng-control.py --alt=0403:6010 -p"
  print ""
  print "\treset the internal state machine of the FPGA"
  print "\t\tsng-control.py -r"
  print ""
  print "\tread back the .bin file, all files will be 64MB (long time)"
  print "\t\tsng-control.py -z outfile.txt"
  print ""
  print "Author: Dave McCoy (dave.mccoy@cospanddesign.com)"
  print ""
